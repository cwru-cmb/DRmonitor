...
def _Status_handler(lines: list[list[str]]) -> dict[str, pd.DataFrame]:
    ...
    for l in lines:
        ...
        while i < len(l):
            try:
                data_groups[l[i]].append([d, l[i + 1]])
            except KeyError:
                data_groups[l[i]] = [[d, l[i + 1]]]
            i += 2
...
def _add_file_to_channels(entry: os.DirEntry, date: str, channels: dict[str, Channel]):
    # if configured for debugging, only skip all channels but CH1 T
    if (config.ONLY_LOAD_CH1_T and not entry.name.startswith('CH1 T')):
        pass
...
def injest_date_dirs(date_dirs: list[os.DirEntry]) -> dict[str, Channel]:
    """
